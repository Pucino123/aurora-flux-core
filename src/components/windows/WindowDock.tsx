  ...
  return createPortal(
    ...
    >
      ...
      <div className="flex items-center gap-1.5 px-2 py-1.5 border-b border-white/8">
        ...
        <span className="flex-1 text-[9px] font-semibold text-white/50 truncate text-center">
          {win.title}
        </span>
      </div>
      {/* Icon placeholder */}
...
function WindowTile({ win }: { win: AppWindow }) {
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {hovered && !isMinimized && (
          <TilePreview win={win} anchorRef={tileRef as React.RefObject<HTMLDivElement>} />
        )}
      </AnimatePresence>
