...
const CouncilAvatar = ({ color, vote, isSpeaking = false, isDebating = false, size = 64, onClick, personalityIndex }: CouncilAvatarProps) => {
  ...
  const getIdleAnimation = () => {
    switch (pi) {
      case 0: return { y: [0, -3, 0], rotate: [-1, 1, -1], scale: [1, 1.02, 1] };
      case 1: return { x: [-1.5, 1.5, -1.5], y: [0, -1, 0], scale: [1, 1.01, 1] };
      case 2: return { rotate: [-2, 0, 2, 0, -2], y: [0, -1, 0, -0.5, 0], scale: [1, 1.01, 1] };
      case 3: return { rotate: [0, -3, 0, 3, 0], y: [0, -1.5, 0] };
      case 4: return { y: [0, -4, 0, -2, 0], scale: [1, 1.03, 1, 1.01, 1] };
      default: return { y: [0, -2, 0], scale: [1, 1.01, 1] };
    }
  };
  ...
  const getLeftArmIdle = () => {
    switch (pi) {
      case 0: return { rotate: [0, -8, 0], y: [0, -1, 0] };
      case 1: return { rotate: [0, -3, 0] };
      case 2: return { rotate: [-5, 5, -5], y: [0, -2, 0] };
      case 3: return { rotate: [0, -12, 0, -12, 0] };
      case 4: return { rotate: [-8, 8, -8], y: [0, -4, 0] };
  ...
  const getRightArmIdle = () => {
    switch (pi) {
      case 0: return { rotate: [0, 3, 0] };
      case 1: return { rotate: [0, 5, 0, 5, 0] };
      case 2: return { rotate: [5, -5, 5] };
      case 3: return { rotate: [0, 8, 0] };
      case 4: return { rotate: [8, -8, 8], y: [0, -3, 0] };
      default: return { rotate: [0, 2, 0] };
    }
  };

  // Aura pulse durations per personality
  const auraDurations = [3.0, 2.6, 2.2, 3.4, 2.0];

  const blinkAnimation = { scaleY: [1, 1, 1, 0.08, 1] };
  ...
  const getEyeIdleAnimation = () => {
    ...
    switch (pi) {
      case 3: return { cx: [size * 0.43, size * 0.44, size * 0.42, size * 0.43] };
      case 2: return { cy: [size * 0.33, size * 0.32, size * 0.33] };
      default: return {};
    }
  };
  ...
  return (
    ...
    >
      {/* Pulsing aura glow behind avatar */}
      <motion.div
        style={{
          position: "absolute",
          inset: -size * 0.12,
      ...
      <motion.svg
        ...
        onClick={onClick}
        className={onClick ? "cursor-pointer relative z-10" : "relative z-10"}
      ...
      >
        ...
        {(isSpeaking || isDebating) && (
          <motion.circle
            ...
            animate={{ r: [size * 0.46, size * 0.48, size * 0.46], opacity: [0.2, 0.5, 0.2] }}
        ...
        <motion.circle
          ...
          style={{ transformOrigin: `${size * 0.43}px ${size * 0.33}px` }}
        ...
        <motion.circle
          ...
          style={{ transformOrigin: `${size * 0.57}px ${size * 0.33}px` }}
          whileHover={{ scaleY: 1.2, scaleX: 1.1 }}
        />
        {/* Eye connector */}
