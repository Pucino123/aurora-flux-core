...
import { X, Send, UserPlus, Users, Plus, Check, AlertCircle, LogOut, Trash2, Smile, Hash, Link, Copy } from "lucide-react";
...
function UserAvatar({ userId, displayName, avatarUrl, size = "sm" }: {
  userId: string; displayName?: string; avatarUrl?: string; size?: "sm" | "md";
}) {
  ...
  if (avatarUrl) {
    return <img src={avatarUrl} alt={displayName || userId} className={`${sizeClass} rounded-full object-cover shrink-0 border border-white/10`} />;
  }
  return (
            ...
            <div className="flex items-center gap-0.5 bg-white/5 rounded-full p-0.5 shrink-0">
              {(["chat", "contacts"] as Tab[]).map((t) => (
                <button key={t} onClick={() => setTab(t)}
                  className={`px-3 py-1 rounded-full text-[11px] font-semibold transition-all ${tab === t ? "bg-white/15 text-white" : "text-white/40 hover:text-white/70"}`}>
            ...
            {teams.length > 0 && (
              <div className="flex items-center gap-1 overflow-x-auto no-scrollbar">
                {teams.map((t) => {
                  ...
                  return (
                    <button key={t.id} onClick={() => { setActiveTeamId(t.id); setTab("chat"); }}
                      className={`relative flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-medium transition-all shrink-0 ${
                        isActive ? "bg-white/20 text-white" : "bg-white/5 text-white/40 hover:bg-white/10 hover:text-white/70"
                      }`}>
                ...
                {/* Quick add team */}
                <button onClick={() => { setTab("contacts"); setShowCreateTeam(true); }}
                  className="flex items-center justify-center w-5 h-5 rounded-full bg-white/5 text-white/30 hover:bg-white/10 hover:text-white/60 transition-all shrink-0">
                  <Plus size={10} />
                </button>
              </div>
            )}
          ...
          {tab === "chat" ? (
                          ...
                          <AnimatePresence>
                            {hoveredMsgId === msg.id && (
                              <motion.div
                                ...
                                className={`absolute ${isMe ? "right-full mr-1" : "left-full ml-1"} top-0 flex items-center gap-0.5 bg-black/70 border border-white/10 rounded-full px-1.5 py-1 backdrop-blur-sm z-10`}>
                                {REACTION_EMOJIS.map((e) => (
                                  <button key={e} onClick={() => toggleReaction(msg.id, e)}
                                    className="text-[13px] hover:scale-125 transition-transform leading-none p-0.5">
                                    {e}
                                  </button>
                                ))}
                              </motion.div>
                              ...
                              return (
                                <button key={emoji} onClick={() => toggleReaction(msg.id, emoji)}
                                  className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[10px] transition-all ${
                                    myReaction ? "bg-white/20 text-white border border-white/20" : "bg-white/5 text-white/50 border border-white/5 hover:bg-white/10"
                                  }`}>
                                  <span>{emoji}</span>
                ...
                <div className="px-4 py-3 border-t border-white/10">
                  {showEmojiPicker && (
                    <div className="mb-2 p-2 bg-white/5 border border-white/10 rounded-xl grid grid-cols-10 gap-1">
                      {INSERT_EMOJIS.map((e) => (
                        <button key={e} onClick={() => insertEmoji(e)} className="text-base hover:scale-125 transition-transform leading-none">{e}</button>
                      ))}
                    </div>
                  )}
            ...
            <motion.div key="contacts" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="flex flex-col">
              ...
              <div className="px-5 py-4 border-b border-white/10 space-y-3">
                ...
                {inviteStatus === "ok" && <p className="text-[11px] text-white/50 flex items-center gap-1"><Check size={11} /> Invited!</p>}
                {inviteStatus === "error" && <p className="text-[11px] text-white/40 flex items-center gap-1"><AlertCircle size={11} /> {inviteError || "User not found"}</p>}

                {/* Invite link */}
                {activeTeamId && (
                  <button onClick={handleCopyInviteLink} disabled={inviteLinkLoading}
                    className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-white/50 hover:text-white/80 text-xs font-medium transition-all disabled:opacity-30">
                    {inviteLinkLoading ? (
              ...
              <div className="px-5 py-4 overflow-y-auto council-hidden-scrollbar max-h-[34vh]">
                <p className="text-[10px] font-semibold text-white/30 uppercase tracking-wider mb-3">Members ({members.length})</p>
                {members.length === 0 ? (
